...
function FriendRemove() {
    ...
    function handleDeleteFriend(index: number, friendID: string) {
        const uid = localStorage.getItem("uid");
        if (!uid) {
            return;
        }
                ...
                {searchedFriends.length > 0 ?
                    searchedFriends.map((_friend, _index) => (
                        <div key={_index} className="relative">
                            ...
                            <img src="/group_delete_picture.svg" className="absolute top-0 right-5 translate-y-[25%] invert hover:contrast-[20%] h-9 w-auto" onClick={() => handleDeleteFriend(_index, _friend.userID)} />
                        </div>
                    )) :
